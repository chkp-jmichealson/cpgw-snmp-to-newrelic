...
#]
#
# PIP Requirements: pip install requests pysnmp
#
...
def main(argv=None):
    ...
    jsonfilename = 'metrics_list.json'
...
#     ]
    ...
    for item in metrics_list:
        ...
        if 'index_oid' in item: 
            count = get_snmp(OPTIONS.snmp_agent_ip, [item['index_oid']], hlapi.CommunityData(OPTIONS.snmp_community)) # integer expected
            print('Count for index OID %s: %s' % (item['index_oid'], count[item['index_oid']]))

            for i in range(1, (count[count.keys()[0]] + 1)): 
                ...
                if metric_data[metric_data.keys()[0]]:
                    ...
                    print('  Value is empty or not found for OID: %s' % (current_metric_oid))
        else:
